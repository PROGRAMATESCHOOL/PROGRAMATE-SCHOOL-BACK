...
const RegisterToAnnouncement = async (req, res) => {
  ...
  const StudentIntoAnnouncement = await Announcement.findOneAndUpdate(
    { nameAnnouncement: nameAnnouncement },
    { $push: { studentsRegistered: idStu } }
  );
  ...
  if (
    logic2 ==
    "Seis tiburones no son completamente ciegos y tres son completamente ciegos"
  ) {
    form_ScoreLogic += 1;
  }
  if (logic3 == "E") {
  ...
  // TOTAL SCORE
  form_ScoreTotal =
    form_ScoreProfile * 0.4 +
    form_ScoreVocation * 0.3 +
    form_ScoreMotivation * 0.15 +
    form_ScoreLogic * 0.15;
  ...
  while (validations == false) {
    if (form_emailPerson == emailGuardian) {
      alert(
        "Este email ya está registrado por un estudiante. Por favor ingrese un correo diferente"
      );
      res.send(
        "Este email ya está registrado por un estudiante. Por favor ingrese un correo diferente"
      );
      ...
      if (phone == phoneGuardian) {
        alert(
          "Este teléfono es el mismo del estudiante. Por favor ingrese un teléfono diferente"
        );
        res.send(
          "Este teléfono es el mismo del estudiante. Por favor ingrese un teléfono diferente"
        );
